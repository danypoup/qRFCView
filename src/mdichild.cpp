...
bool MdiChild::ParseRFCText(const QFile& qFile, QString& qOutput)
{
    ...
    while (!qInStream.atEnd()) {
        ...
        if (iState == PARSER_NORMALTEXT) {
            if ((qRegExpTitle.indexIn(qLine, 0)) != -1) { // Is it a new title?
                // Check title num is correct
                if (m_pTitleModel->ParseTitle(qRegExpTitle.cap(1), qRegExpTitle.cap(2))) { // Insert the HTML anchor
                    qLine = "</PRE><H3><a name=\"Title_" + qRegExpTitle.cap(1) + "\"></a>" + qLine + "</H3><PRE>";
                    //bLineNotAdded=false;
                }
            } else if ((qRegExpBiblio.indexIn(qLine, 0)) != -1) { // Is it a bib ref (shall be at the end of the document
                qLine = "<font color=\"Red\"><a name=\"BibRef_" + qRegExpBiblio.cap(1) + "\"></a>" + qLine;
                iNextState = PARSER_BIBTEXT;
                //bLineNotAdded=false;
            } else if ((qRegExpFooter.indexIn(qLine, 0)) != -1) { // Test if it is the footer line
                qLine = "<font color=\"Grey\">" + qLine + "</font>";
            } else if (qLine.indexOf(QChar('\f')) != -1) { // Test if it is the header line
        ...
        iPrevPos = 0;
        if (iNextState != PARSER_BIBTEXT && (iNextPos = qRegExpReference.indexIn(qLine, iPrevPos)) != -1) { // Format biblio references: "[%d]"
            qLineOutput.clear();
            do {
        ...
        iPrevPos = 0;
        if (iState != PARSER_HEADER && (iNextPos = qRegExpRFC.indexIn(qLine, iPrevPos)) != -1) { // Format RFC references: "RFC %d"
            qLineOutput.clear();
            do {
                // Insert a link to the reference
                //qDebug()<<qRegExpReference.cap(1);
...
bool MdiChild::FindText(QString& qTextToFind, uint32_t iOptionFlags)
{
    ...
    if (!(iOptionFlags & FIND_OPTIONSFLAG_CURSOR)) { // Set the cursor at the beginning
        QTextCursor cursor(m_pTextEdit->textCursor());
...
void MdiChild::Print(QPrinter* qPrinter, bool bAll, int fromPage, int toPage)
{
    ...
    while (!qInStream.atEnd()) {
        ...
        // Change font
        if (iState == PARSER_NORMALTEXT) {
            if (qLine.indexOf(QChar('\f')) != -1) { // Test if it is the header line
